...
-export([extend/2, withdraw/2, get_value/2, set_value/2, set_value/3, delete/2]).
...
set_value(Elements, Struct) ->
    lists:foldl(fun({K, V}, Acc) -> set_value(K, V, Acc) end, Struct, Elements).
...
%% @spec delete(path() | key() | list(), struct()) -> value()
delete(Fields, Doc) when is_list(Fields) ->
    lists:foldl(fun(X, Acc) -> delete(X, Acc) end, Doc, Fields);
